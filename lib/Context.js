...
internals.Context.prototype.getServiceTaskIO = function(activityId) {
  const io = contextHelper.getServiceTaskIO(this.moddleContext, activityId);
  if (!io) return;
  return new mapper.ActivityIO(io, this);
};
...
internals.Context.prototype.getElementService = function(element) {
  ...
  return new mapper.ServiceConnector(contextService);
};
