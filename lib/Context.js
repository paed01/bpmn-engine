...
function Context(processId, moddleContext, environment) {
  ...
  const env = this.environment = environment || Environment();
  this.activity = moddleContext.elementsById[processId];
  this.type = this.activity.$type;

  this.dataObjects = DataObjects(contextHelper.getDataObjects(), contextHelper.getDataObjectReferences(), env);
...
Context.prototype.clone = function(environment) {
  return new Context(this.id, this.moddleContext, environment || this.environment);
};
...
Context.prototype.getActivityIO = function(activityId) {
  ...
  if (!io) return this.getDefaultIO(activityId);

  const ActivityIOType = mapper(io.$type);
  return ActivityIOType(io, this);
};

Context.prototype.getDefaultIO = function(activityId) {
  return ActivityIO(this.contextHelper.getActivityById(activityId), this);
};

Context.prototype.getActivityIOReferences = function(ioSpecification) {
  return this.contextHelper.getActivityIOReferences(ioSpecification);
};
...
Context.prototype.getActivityForm = function(activity) {
  ...
  return new FormType(formData, this);
};
