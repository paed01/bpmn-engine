...
function Context(processId, moddleContext, variablesAndServices) {
  ...
  this.variablesAndServices = Object.assign({variables: {}, services: {}}, variablesAndServices);
...
Context.prototype.getServiceByName = function(serviceName) {
  const serviceDef = this.variablesAndServices.services[serviceName];
...
function Activity(childActivity, context) {
  ...
  const loop = context.getLoopCharacteristics(childActivity.loopCharacteristics);
  ...
  const getVariablesAndServices = context.getVariablesAndServices.bind(context);
  ...
  function getServiceByExpression() {
    ...
    return {
      ...
      execute: (executeOnBehalfOf, message, callback) => {
        const serviceFn = expressions(fnExpression, context.getVariablesAndServices());
  ...
  function resolveExpression(expression, message) {
    const varContext = getVariablesAndServices(message, true);
    return expressions(expression, varContext);
  }
