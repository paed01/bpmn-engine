...
function FlowManager(activity) {
  ...
  let activated, inboundFlows, outboundFlows, immediateInboundDiscarded;
  ...
  function emit(eventName, ...args) {
    ...
    switch (eventName) {
      ...
      case 'all-outbound-discarded':
        resetFlows = true;
        break;
      case 'inbound-discarded':
        if (waitForInboundTaken(eventName, ...args)) return;
        break;
      case 'inbound-taken':
        resetWaitForInboundTaken();
        break;
    }
  ...
  function waitForInboundTaken(...args) {
    if (!inboundFlows.multiple) return;
    if (immediateInboundDiscarded) return;

    debug(`<${id}> wait for any taken inbound`);

    immediateInboundDiscarded = setImmediate(immediateEmit);
    return immediateInboundDiscarded;

    function immediateEmit() {
      debug(`<${id}> no inbound taken`);
      emit(...args);
    }
  }

  function resetWaitForInboundTaken() {
    if (!immediateInboundDiscarded) return;
    clearImmediate(immediateInboundDiscarded);
    immediateInboundDiscarded.unref();
    immediateInboundDiscarded = null;
  }
