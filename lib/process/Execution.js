...
function ProcessExecution(parentApi, context, emit) {
  const {id, children, environment, messageFlows} = context;
  const {type, isSubProcess} = parentApi;
  ...
  function resumeChildren(state) {
    variablesAndServices = environment.getVariablesAndServices();
    ...
    resumeSequences.pre.forEach((childState) => {
      ...
      if (activityApi) activityApi.resume(childState);
    });
  ...
  function getState() {
    const result = {
      ...
      type,
      entered: started,
      environment: environment.getState(true),
  ...
  function getPendingActivities() {
    return pendingExecutions.reduce((result, pe) => {
      const activityApi = getChildActivityApi(pe.activityId).getApi(pe);
      result.push(activityApi);

      if (activityApi.loop) {
        result = result.concat(activityApi.loop);
      }
  ...
  function onChildEvents(activity) {
    activity.on('enter', onChildEnter)
      .on('start', onChildStart)
      .on('wait', onChildWait)
      .on('end', onChildEnd)
      .on('terminate', onChildTerminate)
      .on('cancel', onChildCancel)
      .on('leave', onChildLeave)
      .on('error', onChildError)
      .on('catch', onChildCatch);
  }
  ...
  function onChildLeave(activityApi, childExecutionContext) {
    const {id: childId, type: childType} = childExecutionContext;
    const idx = pendingExecutions.findIndex((c) => c === childExecutionContext);
    if (idx < 0) return complete(new Error(`<${id}> child ${childType} <${childId}> is not running`));

    pendingExecutions.splice(idx, 1);

    debug(`<${id}> left <${childExecutionContext.id}> (${childType}), pending activities ${pendingExecutions.length}`);
