...
Process.prototype.run = function runProcess(onStart) {
  ...
  function onChildEvent(eventName, ...args) {
    const eventArgs = {
      eventName: eventName
    };
    switch (eventName) {
      case 'message':
        eventArgs.message = args[0];
        eventArgs.source = args[1];
        eventArgs.context = args[2];
        break;
      default:
        eventArgs.source = args[0];
        eventArgs.context = args[1];
        break;
    }
  ...
  function emitListenerEvent({eventName, source, context}) {
    const sourceApi = source.getApi ? source.getApi(context) : source;
    listener.emit(`${eventName}-${source.id}`, sourceApi, execution);
    listener.emit(eventName, sourceApi, execution);
  }
};
