...
module.exports = function Service(activityElement, parentContext) {
  ...
  const {environment, getActivityExtensions} = parentContext;
  const {services} = getActivityExtensions(activityElement);
  ...
  function resume(parentApi, state) {
    const serviceApi = activate(parentApi);
    serviceApi.resume(state.services);
    return serviceApi;
  }

  function activate(parentApi, inputContext, executionContext) {
    if ((!services || !services.length) && environment.get('disableDummyService')) return;
    ...
    function getServices() {
      ...
      if (services && services.length) {
        activatedServices = services.map((service) => service.activate(parentApi, inputContext, executionContext));
      } else {
