...
BaseProcess.prototype.execute = function(executionContext, callback) {
  this._debug(`<${this.id}>`, 'execute');
  this.isEnded = false;
  this.context.applyExecutionContext(executionContext.getInput());
  ...
  if (callback) {
    this.completeProcess = this.completeProcess.bind(this, callback);
  }
};

BaseProcess.prototype.completeProcess = function(callback) {
...
BaseProcess.prototype.signal = function(childId, input) {
  const childExecutionIdx = this.waitingActivities.findIndex((a) => a.id === childId);
  if (childExecutionIdx < 0) return;

  this._debug(`<${this.id}>`, `signal <${childId}>`);

  const childExecution = this.waitingActivities.splice(childExecutionIdx, 1)[0];

  if (childExecution.activity.isStartEvent) {
    this.context.applyMessage(input);
    return this.run();
  }

  if (childExecution.activity.isStart && !this.entered) {
    this.run();
  }
...
BaseProcess.prototype.onChildWait = function(activityExecution) {
  this._debug(`<${this.id}>`, `wait for <${activityExecution.id}> (${activityExecution.type})`);
  this.waitingActivities.push(activityExecution);
  emitListenerEvent.call(this, 'wait', activityExecution);
};
