...
module.exports = internals.Process = function(activity, moddleOrParentContext, listener) {
  this.context = getContext(activity, moddleOrParentContext, listener);
...
function getContext(activity, moddleOrParentContext, listener) {
  if (isSubprocess(moddleOrParentContext)) {
    debug(`<${activity.id}>`, 'is most likelly a subProcess');
    return new Context(activity.id, moddleOrParentContext.moddleContext, moddleOrParentContext.listener);
  }

  debug(`<${activity.id}>`, 'is most likelly a main process');
  return new Context(activity.id, moddleOrParentContext, listener);
}
...
internals.Process.prototype.run = function(variables) {
  ...
  this.context.startActivities.forEach((activity) => activity.run(variables));
};
