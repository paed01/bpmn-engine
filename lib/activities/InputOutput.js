...
function getValue(parm, variables) {
  ...
  if (parm.script) {
    return script.execute(parm.script, variables);
  }
}
...
function getParmScript(rawParm) {
  ...
  if (firstVal.$type === 'camunda:script') {
    if (!script.isJavascript(firstVal.scriptFormat)) throw new Error(`Script format ${firstVal.scriptFormat} is unsupported (${rawParm.name})`);
    return script.parse(`${rawParm.name}.io`, firstVal.$body);
  }
}
