...
module.exports = internals.Activity = function(activity, parentContext) {
  ...
  this._debug(`<${this.id}>`, `init${this.isStart ? ' starting element' : ''}`);
};
...
internals.Activity.prototype.cancel = function(rootFlow) {
  this.canceled = true;
  ...
  this.discardAllOutbound(rootFlow);
};
...
internals.Activity.prototype.discard = function(flow, rootFlow) {
  if (this.isEnd) {
    this._debug(`<${this.id}>`, `discarded by <${flow.id}>`);
    return;
  }
  if (!this.entered) this.enter();
...
internals.Activity.prototype.takeAllOutbound = function() {
  if (!this.isEnd) {
    this._debug(`<${this.id}>`, `take all outbound (${this.outbound.length})`);
    this.outbound.forEach((flow) => flow.take());
  }
  this.leave();
};
