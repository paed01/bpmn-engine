...
function execution(message, executionContext) {
  const activity = this;
  ...
  function getInput() {
    if (!inputParameters) {
      return Object.assign({}, message, executionContext);
    }

    const result = {};
    inputParameters.forEach((parm) => {
      result[parm.name] = parm.getInputValue(message, executionContext);
    });
    return result;
  }
  ...
  function getOutput() {
    ...
    outputParameters.forEach((parm) => {
      result[parm.name] = parm.getOutputValue(resultData, executionContext);
    });
...
internals.Activity.prototype.getExecution = function() {
  return execution.apply(this, arguments);
};
