...
Activity.prototype.resume = function(state) {
  ...
  this.execute(executionContext, (err, result, hasOutputParameters) => {
    if (err) {
      this._debug(`<${this.id}> resume error: ${err.message}`, this.listeners('error'));
    ...
    this.complete(getTaskOutput(this.id, hasOutputParameters || executionContext.hasOutputParameters, executionContext.getOutput()), executionContext);
  });
};
...
Activity.prototype.complete = function(output, executionContext) {
  this.taken = true;
  this.emit('end', this, output, executionContext);
...
Activity.prototype.leave = function(executionContext) {
  ...
  this.entered = false;
  setImmediate(() => {
    this.emit('leave', this, executionContext);
  });
};
