...
internals.Flow.prototype.executeCondition = function(variables) {
  const result = script.execute(this.condition, variables);
...
function getCondition(element) {
  ...
  if (!script.isJavascript(element.conditionExpression.language)) throw Error(`Script format ${element.conditionExpression.language} is unsupported (<${element.id}>)`);

  return script.parse(`${element.id}.condition`, element.conditionExpression.body);
}
