...
module.exports = instance.Characteristics = function(activity, parentContext) {
...
instance.Characteristics.prototype.run = function(variables, message, result, callback) {
  if (this.stop) {
    this.completed = true;
  ...
  if (!this.completed && this.characteristics.condition) {
    this.completed = scriptHelper.execute(this.characteristics.condition, variables, message);
    debug('completed condition', this.completed);
  }

  if (!this.completed && this.characteristics.cardinality) {
    this.completed = this.iteration >= this.characteristics.cardinality;
...
instance.Characteristics.prototype.getCollection = function() {
...
instance.Characteristics.prototype.deactivate = function() {
  debug('stopped');
  this.stop = true;
};

instance.Characteristics.prototype.getState = function() {
  const state = {
...
instance.Characteristics.prototype.resume = function(state) {
...
function getLoopCharacteristics(activity) {
  ...
  const cardinality = activity.loopCardinality && Number(activity.loopCardinality.body);
  ...
  if (activity.completionCondition) {
  ...
  if (!isNaN(cardinality)) {
    ...
    if (!characteristics.type) characteristics.type = 'cardinality';
  }
