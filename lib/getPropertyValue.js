...
function iterateProps(base, obj, propertyPath) {
  ...
  const rest = propertyPath.replace(propertyPattern, (match, fn, args, p, prop) => {
    if (prop) {
      result = getNamedValue(obj, prop);
    } else if (fn) {
      result = executeFn(getNamedValue(obj, fn), args, base);
    }
    return '';
  });
...
function executeFn(fn, args, base) {
  let callArguments = [];
  if (args) callArguments = callArguments.concat(args.split(','));
