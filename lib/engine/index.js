...
const validOptions = ['name', 'source', 'moddleContext', 'moddleOptions', 'extensions', 'Logger'];
...
function EngineInstance(options = {}) {
  ...
  if (extensions) {
    for (const extensionName in extensions) {
      ...
      if (extension.moddleOptions) {
        logger.debug(`adding ${extensionName} moddle options`);
  ...
  function execute(executeOptionsOrCallback, executeCallback) {
    ...
    const execution = Execution(engineApi, emitter, {Logger, ...executeOptions});
  ...
  function resurrect(state, resurrectOptions) {
    logger.debug(`<${name}> resurrect ${state.definitions.length} definitions`);
    state.definitions.forEach(({moddleContext}) => moddleContext && addDefinitionByModdleContext(moddleContext));
    if (state.sources) state.sources.forEach(({moddleContext}) => moddleContext && addDefinitionByModdleContext(moddleContext));
  ...
  function getState() {
    ...
    running.forEach((execution) => {
      const {sources: srcs, definitions: ds} = execution.getState();
      state.definitions = state.definitions.concat(ds);
      state.sources = state.sources.concat(srcs);
    });
