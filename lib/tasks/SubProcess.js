...
SubProcess.prototype.activate = function(state, listener) {
  const task = this;
  const id = task.id;
  ...
  function execute(api, executionContext, callback) {
    ...
    const execution = ProcessExecution(subContext, listener, emitter, callback);
  ...
  function completeCallback(executionContext, subEnvironment) {
    return callback;

    function callback(err, ...args) {
      delete state.entered;

      console.log('COMPETED', id, subEnvironment.output)
