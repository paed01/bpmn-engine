...
UserTask.prototype.activate = function(state) {
  ...
  let activityExecution, postponedExecution;
  ...
  function run(message, inboundFlow) {
    ...
    postponedExecution = activityExecution.postpone(completeCallback);
    state.waiting = true;
    emit('wait', activityApi, postponedExecution);
  ...
  function signal(...args) {
    ...
    postponedExecution.signal(...args);
  }
  ...
  function completeCallback(err) {
    ...
    if (err) return emit('error', err, scope, activityExecution);
