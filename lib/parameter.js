...
function Parameter(ioParm) {
  ...
  function getValueType() {
    if (parm.value) {
      return expressions.isExpression(parm.value) ? 'expression' : 'constant';
    }
  ...
  function getList(message, variablesAndServices) {
    ...
    return parm.definition.items.map((entry) => {
      return expressions.isExpression(entry.value) ? expressions(entry.value, Object.assign({}, message, variablesAndServices)) : entry.value;
    }, {});
  }
