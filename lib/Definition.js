...
internals.Definition.prototype.onMessage = function(from, message, via) {
  ...
  if (runningIndex === -1) {
    return targetProcess.run((executionContext) => {
      executionContext.signal(via.targetId, message);
    });
  }
...
internals.Definition.prototype.onEnd = function(process) {
  ...
  if (runningIndex > -1) this.running.splice(runningIndex, 1);
...
function loadProcesses(moddleContext, options) {
  ...
  this.processes = processElements.map((e) => new Process(e, moddleContext, getVariablesAndServices(), options.listener));
  ...
  function getVariablesAndServices() {
    return {
