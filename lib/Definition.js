...
Definition.resume = function(state, resumeOptions, resumeCallback) {
  ...
  const instance = DefinitionInstance(state.moddleContext, null, Environment(executeOptions).resume(state.environment));
...
function DefinitionInstance(moddleContext, options, environment) {
  ...
  function resumeProcesses(env, state) {
    processes = [];
    for (const processId in state.processes) {
      const processState = state.processes[processId];
