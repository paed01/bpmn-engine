...
module.exports = function contextHelper(moddleContext) {
  const references = moddleContext.references;
  const rootHandler = moddleContext.rootHandler;
  const elementsById = moddleContext.elementsById;
  ...
  const definitionId = rootHandler.element.id;
  ...
  function getActivityInputOutput(activity) {
    let input, output;
    if (activity.ioSpecification) {
      console.log(JSON.stringify(references, null, 2))
      console.log(JSON.stringify(activity, null, 2))
      activity.dataOutputAssociations.map((o) => {
        console.log(getDataObjectFromAssociation(o.id))
      })

      // console.log(JSON.stringify(activity.ioSpecification, null, 2))
    }
  ...
  function getDataObjectFromAssociation(associationId) {
    const association = references.find((r) => r.element.$type === 'bpmn:DataOutputAssociation' && r.element.id === associationId && r.property === 'bpmn:targetRef');
    if (!association) return null;

    const potentialRef = elementsById[association.id];
  ...
  function getChildOutputNames(taskId) {
    const contextElement = elementsById[taskId];
