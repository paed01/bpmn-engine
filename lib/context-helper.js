...
pub.getTasksWithoutInbound = function(context, scopeActivityId) {
  const elements = [];

  pub.getActivities(context, scopeActivityId).forEach((element) => {
    if (!pub.hasInboundSequenceFlows(context, element.id)) {
      elements.push(element);
    }
  });
  return elements;
