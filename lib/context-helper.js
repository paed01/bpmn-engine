...
pub.getActivityOutput = function(context, activityId) {
  const output = {};
  const activity = context.elementsById[activityId];
  if (!activity.extensionElements) return output;

  const inputOutput = activity.extensionElements.values.find((v) => v.$type === 'camunda:inputOutput');
  if (!inputOutput) return output;

  inputOutput.$children.filter((io) => io.$type === 'camunda:outputParameter').forEach((o) => {
    output[o.name] = o.$body;
  });
