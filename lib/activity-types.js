...
const exclusiveGateway = {
  execute: function(activityExecution) {
    const outgoingSequenceFlows = activityHelper.getSequenceFlows(activityExecution.activityDefinition);

    debug(`exclusiveGateway ${activityExecution.activityDefinition.id} has ${outgoingSequenceFlows.length} outgoing`);
    ...
    if (!sequenceFlowToTake) {
      if (!defaultFlow) {
        throw new Error(`Cannot determine outgoing sequence flow for exclusive gateway '${activityExecution.activityDefinition.id}': All conditions evaluate to false and a default sequence flow has not been specified.`);
      } else {
