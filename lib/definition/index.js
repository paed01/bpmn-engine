...
Definition.resurrect = resurrectDefinition;
Definition.resume = resumeDefinition;

function resumeDefinition(state, resumeOptions, resumeCallback) {
  const [options, callback] = getOptionsAndCallback(resumeOptions, resumeCallback);
  const instance = resurrectDefinition(state, options);
  setImmediate(instance.resume, state, options, callback);
  return instance;
}

function resurrectDefinition(state, resumeOptions) {
  const environment = Environment(resumeOptions).resume(state.environment);
  const instance = DefinitionInstance(state.moddleContext, null, environment);
  instance.resurrect(state);
  return instance;
}
...
function DefinitionInstance(moddleContext, options, environment) {
  ...
  const id = contextHelper.getDefinitionId();
  ...
  function resurrect(state, resumeOptions) {
    debug(`<${id}> resurrect`);

    if (warnings.length) return emit('error', warnings[0]);

    definitionApi.environment = environment = environment.resume(state.environment);
    if (resumeOptions && resumeOptions.listener) {
      environment.setListener(resumeOptions.listener);
    }
