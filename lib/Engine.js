...
function loadProcesses(context, variables, listener) {
  this.processElements = contextHelper.getProcesses(context);
  this.entryPointId = (this.processElements.find((e) => e.isExecutable) || {}).id;

  debug(`found ${this.processElements.length} processes. Start with <${this.entryPointId}>`);

  this.processes = this.processElements.map((e) => new mapper.Process(e, context, listener, variables));
