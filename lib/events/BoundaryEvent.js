...
BoundaryEvent.prototype.activate = function(state) {
  const debug = Debug('bpmn-engine:bpmn:boundaryevent');
  const scope = this;
  const id = scope.id;
  ...
  function getOutput() {
    if (!boundEventExecution) return {};
    return boundEventExecution.getOutput();
  }
  ...
  function executeAllDefinitions() {
    eventExecutions = getEvents().map((event) => {
      return event.init(boundEventExecution);
    });
