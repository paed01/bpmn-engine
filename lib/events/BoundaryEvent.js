...
module.exports = function BoundaryEvent(activity, parentContext) {
  const {id, type, eventDefinitions, flows} = activity;
  const debug = Debug(`bpmn-engine:${type.toLowerCase()}`);

  const {environment, getAttachedToActivity} = parentContext;
  let attachedTo;
  let cancelActivity = activity.cancelActivity;
  if (cancelActivity === undefined) cancelActivity = true;

  let loadedEventDefinitions, attachedToListener, resumedState;

  const activityApi = Object.assign(activity, {
    cancelActivity,
    getApi,
    getState,
    getEventDefinitions,
  ...
  function getEventDefinitions() {
    ...
    if (!eventDefinitions) {
      ...
      loadedEventDefinitions = eventDefinitions.reduce((result, ed) => {
        const eventDefinition = EventDefinition(activity, ed, parentContext);
